...
class BookingScreenMEALS extends StatefulWidget {
  Food food;

  BookingScreenMEALS({super.key, required this.food});

  @override
  _BookingScreenMEALSState createState() => _BookingScreenMEALSState();
}

class _BookingScreenMEALSState extends State<BookingScreenMEALS> {
  ...
  void initState() {
    ...
    _getUser();
    selectTime(context);
    String priceString = widget.food.Price.join(", ");
  ...
  int selectedNumber = 1; // Store selected number

  void _updatePrice() {
    if (selectedNumber != null) {
      // Assuming you want to multiply the selected number with the first price from the list
      // Convert the first price to an integer (assuming the price list is not empty)
      if (widget.food.Price.isNotEmpty) {
        String firstPriceString =
            widget.food.Price[0]; // Get the first price as a string
        int firstPrice = int.tryParse(firstPriceString) ??
            0; // Convert string to int (default to 0 if it fails)

        // Multiply selected number with the price
        int price = selectedNumber! * firstPrice;
        _PriceController.text = '$price L.E '; // Display the result
      }
    }
  }
                  ...
                  child: Column(
                    children: [
                        ...
                        children: [
                          IconButton(
                            onPressed: () {
                              if (selectedNumber > 1) {
                                setState(() {
                                  selectedNumber--;
                                  _updatePrice(); // تقليل العدد وتحديث السعر
                                });
                              }
                            },
                          ...
                          IconButton(
                            onPressed: () {
                              setState(() {
                                selectedNumber++;
                                _updatePrice(); // زيادة العدد وتحديث السعر
                              });
                            },
                      ...
                      TextFormField(
                        ...
                        enabled: false, // لجعل الحقل غير قابل للتعديل
                      ),
                      // TextFormField(
                      //   controller: _PriceController,
                      //   validator: (value) {
                      //     if (value!.isEmpty) return 'Please enter Price Food';
                      //     return null;
                      //   },
                      //   keyboardType:
                      //       TextInputType.number, // لضمان الإدخال الرقمي
                      //   style: const TextStyle(
                      //       fontSize: 20,
                      //       fontWeight: FontWeight.bold,
                      //       color: Colors.black),
                      //   textAlign: TextAlign.left, // لجعل النص يبدأ من اليسار
                      //   decoration: InputDecoration(
                      //     contentPadding: const EdgeInsets.only(
                      //         left: 20, top: 10, bottom: 10),
                      //     border: const OutlineInputBorder(
